...
const checkEnvCorrect = (config, mode) => {
    const keys = ['name', 'host', 'username', 'distPath', 'webDir', 'script']
...
const backupRemoteFile = async (envConfig) => {
    try {
        ...
        await ssh.execCommand(`echo ${bakDir}/last-dist.zip > back.txt`, { cwd: webDir })
        succeed(`备份成功 备份至 ${bakDir}/last-dist.zip`)
    } catch (e) {
        error(e)
        process.exit(1)
    }
...
const unzipRemoteFile = async (envConfig) => {
    try {
        ...
        await ssh.execCommand(`unzip -o ${remoteFileName} -d ${webDir} && rm -r ${remoteFileName}`, { cwd: '/' })
